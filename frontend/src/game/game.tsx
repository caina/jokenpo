...
export const Game: FunctionComponent<GameType> = () => {
  ...
  const autoBet = () => {
    ...
    setMoves([]);
    fetcher
      .postData(`/api/bet/auto`)
      .then((data) => setMoves(data.map((it: Move) => new Move({ ...it }))))
      .catch(console.error)
      .finally(() => {
        autoBetTimeout = setTimeout(() => autoBet(), A_SECOND + A_SECOND);
      });
  };
    ...
    <section id="game-container">
      ...
      <div id="game-container__display">
        <GameCardComponent
          ...
          win={player1.win}
          selected={player1.move}
        />
        <h3> {timer === 0 ? "VS" : timer} </h3>
        <GameCardComponent
          label={player2.player}
          win={player2.win}
          selected={player2.move}
        />
      </div>
